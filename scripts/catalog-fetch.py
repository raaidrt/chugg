...
parser.add_argument('--directory', type=Path, required=True, help='Local directory for source TSV and PGN files')
parser.add_argument('--download', action='store_true', help='Download public sources into the directory')
parser.add_argument('--rebuild', action='store_true', help='Replace the catalog and counts with the original reference snapshot')
args = parser.parse_args()
args.directory.mkdir(parents=True, exist_ok=True)
for source in SOURCES['openingTables'] + SOURCES['gameArchives']:
    target = args.directory / source['filename']
    if args.download:
        data = subprocess.check_output(['curl', '--fail', '--location', '--silent', '--show-error', '--max-time', '120', source['url']])
...
for source in SOURCES['openingTables']:
    with (args.directory / source['filename']).open() as file:
        upstream_rows.update((row['eco'], row['name'], row['pgn']) for row in csv.DictReader(file, delimiter='\t'))
...
if args.rebuild:
    env = dict(os.environ, CATALOG_SOURCE=SOURCES['popularitySource'],
               CATALOG_DESCRIPTION=SOURCES['popularityDescription'], CATALOG_DATE=SOURCES['retrievedAt'])
    subprocess.run(['node', str(ROOT / 'scripts/catalog-build.mjs'),
